...
pub fn spawn_skybox(
    ...
    let sky_mesh = meshes.add(Mesh::try_from(bevy::prelude::Sphere { radius: 1000.0 }).unwrap());
...
pub fn spawn_sun_and_moon(
    ...
    let sun_mesh = meshes.add(Mesh::try_from(bevy::prelude::Sphere { radius: 20.0 }).unwrap());

    // Create moon mesh (smaller sphere)
    let moon_mesh = meshes.add(Mesh::try_from(bevy::prelude::Sphere { radius: 15.0 }).unwrap());
