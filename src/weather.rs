...
impl WeatherType {
    ...
    pub fn has_precipitation(&self) -> bool {
        matches!(
            self,
            WeatherType::Rain | WeatherType::HeavyRain | WeatherType::Thunderstorm | WeatherType::Snow | WeatherType::HeavySnow
        )
    }

    /// Check if this weather type is a storm
    pub fn is_storm(&self) -> bool {
        matches!(self, WeatherType::Thunderstorm | WeatherType::HeavyRain | WeatherType::HeavySnow)
    }
...
pub fn update_weather_system(
    ...
    if weather_system.transition_progress < 1.0 {
        ...
        weather_system.cloud_coverage = current_coverage + (target_coverage - current_coverage) * weather_system.transition_progress;
        weather_system.cloud_density = current_density + (target_density - current_density) * weather_system.transition_progress;
    ...
    if weather_system.time_since_weather_change > weather_system.min_weather_duration
        ...
        && !weather_system.target_weather.is_storm() // Don't change away from storms too quickly
    {
...
fn change_weather_randomly(weather_system: &mut WeatherSystem) {
    ...
    let new_weather = possible_weather_types[rand::random::<usize>() % possible_weather_types.len()];
    ...
    println!("🌦️  Weather changing from {} to {}", current_weather.display_name(), new_weather.display_name());
}
...
fn update_environmental_conditions(game_time: &crate::time::GameTime, weather_system: &mut WeatherSystem) {
...
pub fn update_weather_particles(
    ...
    match weather_system.target_weather {
        WeatherType::Rain | WeatherType::HeavyRain | WeatherType::Thunderstorm => {
            update_rain_particles(&time, &weather_system, &weather_effects, &mut particle_query);
        }
        WeatherType::Snow | WeatherType::HeavySnow => {
            update_snow_particles(&time, &weather_system, &weather_effects, &mut particle_query);
        }
        _ => {
...
fn update_rain_particles(
    ...
    for mut transform in particle_query.iter_mut() {
        ...
        // Apply wind
        transform.translation.x += weather_system.wind_direction.x * wind_influence * time.delta_secs();
        transform.translation.z += weather_system.wind_direction.z * wind_influence * time.delta_secs();
...
fn update_snow_particles(
    ...
    for mut transform in particle_query.iter_mut() {
        ...
        transform.translation.y -= fall_speed * time.delta_secs() * (0.8 + rand::random::<f32>() * 0.4);
...
pub fn update_lightning_effects(
    ...
    if weather_effects.time_since_last_lightning >= weather_effects.lightning_frequency {
        ...
        if rand::random::<f32>() < 0.3 { // 30% chance per interval
...
pub fn display_weather_info(
    ...
    if game_time.current_time % 5.0 < 0.1 { // Display every 5 seconds
        println!("🌦️  Weather Info:");
        println!("   Current: {}", weather_system.current_weather.display_name());
        println!("   Target: {}", weather_system.target_weather.display_name());
        println!("   Transition: {:.1}%", weather_system.transition_progress * 100.0);
        println!("   Cloud Coverage: {:.1}%", weather_system.cloud_coverage * 100.0);
        println!("   Cloud Density: {:.1}%", weather_system.cloud_density * 100.0);
        println!("   Precipitation: {:.1}%", weather_system.precipitation_intensity * 100.0);
        println!("   Temperature: {:.1}°C", weather_system.temperature);
        println!("   Humidity: {:.1}%", weather_system.humidity * 100.0);
        println!("   Wind: {:.1} m/s {:?}", weather_system.wind_speed, weather_system.wind_direction);
        println!("   Time since change: {:.1}s", weather_system.time_since_weather_change);
    }
}